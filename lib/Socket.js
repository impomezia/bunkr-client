...
const ua           = require('default-user-agent')('bunkr-client', require('../package.json').version);
...
class Socket extends EventEmitter {
  ...
  connect(url, options) {
    ...
    this._sockjs = new SockJS(this._url, undefined, {
      transportOptions: {
        websocket: {
          headers: { 'user-agent': options.ua || ua }
        }
      }
    });
